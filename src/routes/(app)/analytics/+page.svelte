<script lang="ts">
	...
	const statTiles = $derived([
		{
			label: m.analytics_totalEntries(),
			value: data.generalStats.totalEntries,
			icon: BookOpen,
			tint: 'bg-primary/15 text-primary'
		},
		{
			label: m.analytics_totalMeals(),
			value: data.generalStats.totalMeals,
			icon: ChefHat,
			tint: 'bg-accent/15 text-accent'
		},
		{
			label: m.analytics_avgPerWeek(),
			value: data.generalStats.averageEntriesPerWeek,
			icon: TrendingUp,
			tint: 'bg-chart-2/15 text-chart-2'
		},
		{
			label: m.analytics_totalCategories(),
			value: data.generalStats.totalCategories,
			icon: Tags,
	...
	const coverage = $derived([
		{
			label: m.analytics_entriesWithPhotos(),
			percent: photoPercentage,
			count: data.generalStats.entriesWithPhotos,
			color: 'var(--primary)'
		},
		{
			label: m.analytics_entriesWithNotes(),
			percent: notesPercentage,
			count: data.generalStats.entriesWithNotes,
			color: 'var(--chart-2)'
		}
	]);
	...
	const maxMonthlyCount = $derived(
		data.monthlyStats.length > 0 ? Math.max(...data.monthlyStats.map((m) => m.count), 1) : 1
	);
</script>
	...
	{#if data.generalStats.totalEntries > 0}
		<div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
			{#each statTiles as tile (tile.label)}
				<div
					class="flex flex-col gap-4 rounded-3xl border border-border/60 bg-card p-4 shadow-soft"
				>
		...
		{#if data.monthlyStats.length > 0}
			<section class="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
				{@render sectionTitle(
					...
					m.analytics_monthlyActivityDescription()
				)}
						...
						<div class="group flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-2">
							...
							<div
								...
								title="{getMonthName(
									month.month,
									month.year
								)} {month.year}: {month.count} {m.analytics_entries()}"
							></div>
							<span
								class="w-full truncate text-center text-[11px] font-semibold text-muted-foreground capitalize"
							>
		...
		<div class="grid gap-4 md:grid-cols-2">
			<section class="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
				{@render sectionTitle(
					...
					m.analytics_topMealsDescription()
				)}
			...
			<section class="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
				{@render sectionTitle(
					...
					m.analytics_categoryUsageDescription()
				)}
							...
							<div>
								<div class="mb-1.5 flex items-baseline justify-between gap-2">
									...
									<span class="shrink-0 text-xs font-bold"
										>{m.analytics_times({ count: stat.entryCount })}</span
									>
								</div>
			...
			<section class="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
				{@render sectionTitle(
					...
					m.analytics_photoStatsDescription()
				)}
					...
					{#each coverage as item (item.label)}
						<div class="flex flex-col items-center rounded-2xl bg-secondary/60 p-4 text-center">
							<div
								...
								style="background: conic-gradient({item.color} {item.percent *
							...
							<p class="text-xs text-muted-foreground">
								{item.count}
								{m.analytics_of()}
								{data.generalStats.totalEntries}
								{m.analytics_entries()}
							</p>
						</div>
					{/each}
			...
			<section class="rounded-3xl border border-border/60 bg-card p-5 shadow-soft">
				{@render sectionTitle(
					...
					m.analytics_timeframeDescription()
				)}
					...
					>
						<div class="mb-2 flex items-center gap-2">
							...
							{#if dayOfWeek === TODAY_DAY_OF_WEEK}
								<span
									class="rounded-full bg-primary px-2 py-0.5 text-[11px] font-bold text-primary-foreground"
								>
									{m.common_today()}
								</span>
							{/if}
							...
							<div class="flex flex-wrap gap-1">
								{#each patterns as pattern, patternIndex (patternIndex)}
									<Tooltip.Root>
										<Tooltip.Trigger>
											<span
												class="rounded-full bg-card px-2.5 py-0.5 text-xs font-semibold shadow-soft"
											>
