<script lang="ts">
	...
	let title = $state(data.meal.title);
	let defaultNotes = $state(data.meal.defaultNotes || '');
	let selectedCategoryIds = $state<string[]>(data.meal.categories.map((c) => c.id));
			...
			<form
				...
				use:enhance={({ formData, cancel }) => {
					...
					if (!photoFile && !photoPreview) {
						formData.append('defaultPhotoUrl', '');
					} else if (photoPreview && !photoFile && data.meal.defaultPhotoUrl) {
						formData.append('defaultPhotoUrl', data.meal.defaultPhotoUrl);
					}
